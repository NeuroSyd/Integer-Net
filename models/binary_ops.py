'''
Clone binary_ops.py file from https://github.com/DingKe/nn_playground/tree/master/xnornet
'''
...
def integerize(W, H=8.0, axis=None, keepdims=False):
    # Wb = round_through(K.clip(W,-2**H,2**H-1))
    # Wa = _mean_abs(W, axis, keepdims)
    # return Wa, Wb
    Wa = 2.0**H/K.max(K.abs(W))
    Wb = round_through(W*Wa)
    return Wa, Wb
